...
describe('LoginForm Form ', () => {
  it('renders correctly', async () => {
    setup(<LoginForm status={AuthStatusEnum.IDLE} />);
    expect(await screen.findByText(/Login/i)).toBeOnTheScreen();
  });

  it('should display required error when values are empty', async () => {
    const { user } = setup(<LoginForm status={AuthStatusEnum.IDLE} />);
  ...
  it('Should call LoginForm with correct values when values are valid', async () => {
    const { user } = setup(
      <LoginForm onSubmit={onSubmitMock} status={AuthStatusEnum.LOADING} />,
    );
